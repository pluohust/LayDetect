...
bool IsAWriteAAccess(map<ADDRINT, WriteAccessInf> &AccessFrameOne, map<ADDRINT, WriteAccessInf> &AccessFrameTwo)
{
	...
	for(ItOne=AccessFrameOne.begin();ItOne!=AccessFrameOne.end();ItOne++)
	{
		...
		{
			if(((ItOne->second).Write&&(ItTwo->second).Access)||((ItOne->second).Access&&(ItTwo->second).Write))
			{
				return true;
			}
		}
...
/*			map<UINT32, long>::iterator blockVC;
		...
		else
		{
			...
			{
				if(IsAWriteAAccess((findlockinNew->second).SharedMemoryAccess,(findlockinOld->second).SharedMemoryAccess))
				{
