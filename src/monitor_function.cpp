...
VOID ImageLoad(IMG img, VOID *v)
{
	...
	RTN rtngettimeofday = RTN_FindByName(img, "gettimeofday");
	if ( RTN_Valid( rtngettimeofday ))
	{
		RTN_Open(rtngettimeofday);
		RTN_InsertCall(rtngettimeofday, IPOINT_BEFORE, AFUNPTR(Beforegettimeofday), IARG_END);
		RTN_Close(rtngettimeofday);
	}
